...
namespace EulerProblems {
	class Program {
		...
		static int LongestPath(int row, int index) {
			if(row >= Triangle.Length) return 0;
			return Triangle[row][index] + Math.Max(LongestPath(row + 1, index), LongestPath(row + 1, index + 1));
		}

		static void Main(string[] args) {

			Console.WriteLine(LongestPath(0, 0));
